...
async function viaClaudeRank(query: string, items: RankItem[]): Promise<string[]> {
  ...
  const response = await client.messages.create({
    ...
    system:
      'Ты — поисковый ранкер маркетплейса. Дан запрос пользователя (русский/казахский/английский) и каталог товаров. ' +
      'Верни id товаров, которые семантически подходят запросу (по смыслу, не по подстроке), максимум 12, по убыванию релевантности. Ничего не подходит — верни пустой массив.',
